...
public final class AzureBlobStorageTestAccount {
   ...
   * 
   * @param currentRecord
   *          The metrics record to check.
   * @return
   */
  private boolean wasGeneratedByMe(MetricsRecord currentRecord) {
    String myFsId = fs.getInstrumentation().getFileSystemInstanceId()
        .toString();
  ...
  public CloudAppendBlob getAppendBlobReference(String blobKey)
      throws Exception {
    return container.getAppendBlobReference(String.format(blobKey));
  }
    ...
    if (!metricsConfigSaved) {
      new org.apache.hadoop.metrics2.impl.ConfigBuilder()
          ...
          .save(TestMetricsConfig
              .getTestFilename("hadoop-metrics2-azure-file-system.properties"));
  ...
  private static void configurePageBlobDir(Configuration conf) {
    if (conf
        .get(AzureNativeFileSystemStore.KEY_PAGE_BLOB_DIRECTORIES) == null) {
  ...
  private static void configureAtomicRenameDir(Configuration conf) {
    if (conf.get(
        AzureNativeFileSystemStore.KEY_ATOMIC_RENAME_DIRECTORIES) == null) {
  ...
  public static AzureBlobStorageTestAccount createForEmulator()
    ...
    container = account.createCloudBlobClient()
  ...
  public static AzureBlobStorageTestAccount createOutOfBandStore(
    ...
    AzureFileSystemInstrumentation instrumentation = new AzureFileSystemInstrumentation(
        conf);
  ...
  private static URI createAccountUri(String accountName)
    ...
    return new URI(
        WASB_SCHEME + ":" + PATH_DELIMITER + PATH_DELIMITER + accountName);
  }
  ...
  public static AzureBlobStorageTestAccount createAnonymous(
    ...
    if (null == container || !container.exists()) {
      final String errMsg = String.format(
          "Container '%s' expected but not found while creating SAS account.");
  ...
  private static CloudBlockBlob primeRootContainer(CloudBlobClient blobClient,
    ...
    // must be lower case.
    CloudBlobContainer container = blobClient
        .getContainerReference("https://" + accountName + "/" + "$root");
    container.createIfNotExists();
